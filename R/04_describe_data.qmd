...
```{r}
tibble(mean = gene_means_T2D) |>
  ggplot(aes(x = mean)) +
  geom_histogram(bins = 50, fill = "red", alpha = 0.6, color = "black") +
  theme_minimal() +
  labs(
    title = "Histogram of mean gene expression (T2D)",
...
```{r}
tibble(mean = gene_means_none_T2D) |>
  ggplot(aes(x = mean)) +
  geom_histogram(bins = 50, fill = "blue", alpha = 0.6, color = "black") +
  theme_minimal() +
  labs(
    title = "Histogram of mean gene expression (None-diabetic)",
    x = "Mean gene expression",
    y = "Number of genes"
  )
